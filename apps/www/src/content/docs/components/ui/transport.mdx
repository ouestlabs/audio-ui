...
<Preview name="transport-vertical-demo" />

When using `orientation="vertical"`, wrap `Transport` in a container with explicit height (for example `h-40`).


## Source package

<Callout variant="info">
  For low-level composition (`Transport.Root`, `Slider`, `Track`, `BufferedRange`, `Range`, `Thumb`), see [@audio-ui/react on npm](https://www.npmjs.com/package/@audio-ui/react).
</Callout>
