...
- **Queue dialog:** Search, select/play, remove, and clear actions in one surface.
- **Adaptive reordering:** Queue drag-and-drop is active when not filtering.
- **Composable controls:** Shuffle, repeat, and preferences can be combined per layout needs.
...
A button component with composed tooltip behavior, used internally by queue components.
...
<Callout variant="info">
  ...
  manages the audio playback lifecycle using the <code>useAudio()</code> hook
