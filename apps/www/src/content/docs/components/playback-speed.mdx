---
title: Audio Playback Speed
description: Control playback speed with a dropdown menu selector.
---

A dropdown control for playback speed (0.5x to 2x).

<Preview name="playback-speed-demo" />

## Installation

<CodeTabs>
  <TabsList>
    <TabsTrigger value="cli">CLI</TabsTrigger>
    <TabsTrigger value="manual">Manual</TabsTrigger>
  </TabsList>
  <TabsContent value="cli">
    <Install component="playback-speed" />
  </TabsContent>

  <TabsContent value="manual">
    <Steps>
      <Step>Install the runtime dependencies:</Step>
        <Install pkg="@phosphor-icons/react" />
      <Step>Install the required shadcn/ui primitives:</Step>
        <Install shadcn="button dropdown-menu tooltip" />
      <Step>Install the required registry components:</Step>
        <Install component="store html use-audio" />

      <Step>Copy and paste the following code into your project.</Step>
        <Source name="playback-speed" title="components/audio/playback-speed.tsx" />
      <Step>Copy the required store file.</Step>
        <Source name="store" title="lib/audio-store.ts" />
      <Step>Copy the audio lib file.</Step>
        <Source name="html" title="lib/html-audio.ts" />
      <Step>Copy the useAudio hook file.</Step>
        <Source name="use-audio" title="hooks/use-audio.ts" />
      <Step>Update the import paths to match your project setup.</Step>
    </Steps>

  </TabsContent>
</CodeTabs>

## Setup

Wrap your app with [Audio Provider](/docs/components/provider) at the root level.

```tsx showLineNumbers
import { AudioProvider } from "@/components/audio/provider";
```

```tsx showLineNumbers
export default function RootLayout({ children }) {
  return <AudioProvider tracks={initialTracks}>{children}</AudioProvider>;
}
```

## Usage

```tsx showLineNumbers
import { AudioPlaybackSpeed } from "@/components/audio/playback-speed";
```

### Basic

```tsx
<AudioPlaybackSpeed />
```

### Icon

With `size="icon"`, only the speed label is shown (no gauge icon):

```tsx
<AudioPlaybackSpeed size="icon" variant="ghost" />
```

### Custom Speeds

```tsx
<AudioPlaybackSpeed
  speeds={[
    { value: 0.5, label: "0.5x" },
    { value: 1, label: "1x" },
    { value: 1.5, label: "1.5x" },
    { value: 2, label: "2x" },
  ]}
/>
```

## API Reference

### AudioPlaybackSpeed

#### Props

| Prop        | Type                                    | Default                    | Description                                    |
| ----------- | --------------------------------------- | -------------------------- | ---------------------------------------------- |
| `speeds`    | `readonly { value: number; label: string }[]` | `PLAYBACK_SPEEDS` | Speed options array. Default: 0.5x, 0.75x, 1x, 1.25x, 1.5x, 2x |
| `size`      | `ButtonSize`                            | -                          | Button size. `"icon"` hides gauge icon.        |
| `variant`   | `ButtonVariant`                         | `"outline"`                | Button variant.                                |
| `className` | `string`                                | -                          | Additional CSS classes.                        |

<Callout variant="info">
  <strong>Required:</strong> This component needs
  [Audio Provider](/docs/components/provider) (or an equivalent audio store).
</Callout>

## Examples

### In Player

```tsx
<AudioPlayer>
  <AudioPlayerControlBar>
    <AudioPlayerControlGroup>
      <AudioPlayerPlay />
      <AudioPlayerSeekBar />
    </AudioPlayerControlGroup>
    <AudioPlayerControlGroup>
      <AudioPlaybackSpeed />
      <AudioPlayerVolume />
    </AudioPlayerControlGroup>
  </AudioPlayerControlBar>
</AudioPlayer>
```

### Custom Labels

```tsx
<AudioPlaybackSpeed
  speeds={[
    { value: 0.75, label: "Slow" },
    { value: 1, label: "Normal" },
    { value: 1.5, label: "Fast" },
  ]}
/>
```

## Related

- [Audio Player](/docs/components/player) - Player component system
- [Audio Store](/docs/lib/audio-store) - Playback state management
- [Audio Provider](/docs/components/provider) - Audio context provider

## Changelog

### 2025-12-24 useAudio integration and live stream improvements

- **Changed**: Now uses `useAudio()` hook to access `htmlAudio` singleton
- **Improved**: Live stream detection uses `htmlAudio.isLive()` method
- **Fixed**: Playback rate changes are properly prevented for live streams

