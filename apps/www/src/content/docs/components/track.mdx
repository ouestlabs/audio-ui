...
- **Single row or full list:** Use `AudioTrack` for one item, `AudioTrackList` for queue/list rendering.
- **Mode-based behavior:** `mode="static"` for fixed lists, `mode="sortable"` for drag-and-drop.
- **Composable presentation:** `media` and `actions` variants replace multiple boolean display flags.
...
- Optional drag-and-drop (`mode="sortable"`) - reordering updates the queue only when not filtered and when `tracks` is not provided.
...
<Callout variant="info">
  ...
  - **Track Display:** Cover images are taken from <code>track.artwork</code> or{" "}
    ...
    is hidden. Live detection uses <code>htmlAudio.isLive()</code> from the{" "}
    <code>useAudio()</code> hook.
</Callout>
...
<Callout variant="warning">
  <strong>Sortable Behavior:</strong> When <code>mode="sortable"</code> is enabled,
...
Keep <code>mode="static"</code> while filtering to avoid confusing
reordering behavior.
...
<Callout variant="info">
  ...
  - **AudioProvider Requirement:** <code>AudioProvider</code> (or the project's
    ...
    playback state and lifecycle using the <code>useAudio()</code> hook, making
    it essential for all audio components.
