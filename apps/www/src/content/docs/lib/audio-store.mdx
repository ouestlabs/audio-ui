...
Common audio track object. See [HTML Audio](/docs/lib/html-audio#track) for full definition.
...
### 2025-12-24 Provider integration and state management improvements

- **Changed**: Store now focuses solely on state management (no direct audio element access)
- **Improved**: Better separation of concerns between store and provider
- **Improved**: Provider handles all direct audio interactions via `useAudio()` hook
- **Fixed**: State synchronization between audio element and store is more reliable
