...
const { BASES: ALL_BASES } = await import("../registry/bases.ts");
const { isCatalogItemName } = await import("../lib/catalog-name.ts");

// Base-only launch: audio components only exist for the Base UI base.
const BASES = ALL_BASES.filter((b: { name: string }) => b.name === "base");

// Style names only (`registry/styles.tsx` also holds inline JSX icon SVGs,
// which tsx's CJS loader can't evaluate under this Bun-hoisted toolchain;
// only `.name` is needed here — see registry/styles/style-*.css for the set).
const STYLES = [
  { name: "vega" },
  { name: "nova" },
  { name: "maia" },
...
async function getMetadata(base: string): Promise<MetadataData> {
  ...
  try {
    const mod = await import(
      `../registry-audio/bases/${base}/audio/_registry.ts`
    );
    for (const item of mod.audio || []) metadata[item.name] = item;
  } catch {}

  // Load hooks
  try {
    const mod = await import(
      `../registry-audio/bases/${base}/hooks/_registry.ts`
    );
  ...
  try {
    const mod = await import(
      `../registry-audio/bases/${base}/components/_registry.ts`
    );
  ...
  try {
    const mod = await import(
      `../registry-audio/bases/${base}/lib/_registry.ts`
    );
    for (const item of mod.lib || []) metadata[item.name] = item;
  } catch {}
