...
const fs =
  typeof window === "undefined" ? eval('require("fs").promises') : null;
...
function transformAudioPath(filePath: string, _styleName: string): string {
  if (filePath.includes("/__generated/")) {
    return filePath.replace(
      /registry-audio\/bases\/__generated\/[^/]+\//,
      (match) => {
        if (match.includes("base-")) {
          return "registry-audio/bases/base/";
        }
        if (match.includes("radix-")) {
          return "registry-audio/bases/radix/";
        }
        return match;
      }
    );
