...
export function getMDXComponents(components?: MDXComponents): MDXComponents {
  return {
    ...
    h2: ({ className, ...props }: React.ComponentProps<"h2">) => (
      <h2
        className={cn(
          "[&+.steps]:!mt-0 [&+.steps>h3]:!mt-4 [&+h3]:!mt-6 [&+p]:!mt-4 mt-10 scroll-m-28 font-medium font-site-heading text-xl tracking-tight first:mt-0 lg:mt-16 [&+]*:[code]:text-xl",
          className
        )}
        id={props.children
          ?.toString()
    ...
    h3: ({ className, ...props }: React.ComponentProps<"h3">) => (
      <h3
        className={cn(
          "[&+p]:!mt-4 mt-12 scroll-m-28 font-medium font-site-heading text-lg tracking-tight *:[code]:text-xl",
          className
        )}
        {...props}
      />
    ...
    code: ({
      ...
      if (typeof props.children === "string") {
        return (
          <code
            className={cn(
              "site-rounded-md relative break-words bg-site-muted px-[0.3rem] py-[0.2rem] font-mono text-[0.8rem] outline-none",
              className
            )}
            {...props}
          />
